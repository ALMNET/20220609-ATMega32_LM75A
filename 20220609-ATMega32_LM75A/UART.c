...
FILE  UART_Stream = FDEV_SETUP_STREAM(putch0, getch0, _FDEV_SETUP_RW);


static int putch0(char c, FILE *stream) {
	while(UART_write_buff_full() == 1);
	UART_putc(c);
	return 0;
}
